...
export default function SpecialistLoginModal({
  ...
  function handleSignIn() {
    if (!selected) return
    setCurrentSpecialist({ id: selected.id, name: selected.name, role: selected.role })
    onClose?.()
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="relative bg-surface-card border border-outline rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-8">
        ...
        <p className="text-sm text-on-surface-muted mb-6">Select a demo account to continue</p>
            ...
            {selected ? (
              <>
                ...
                <span className="flex-1 min-w-0">
                  <span className="block font-medium text-on-surface">{selected.name}</span>
                  <span className="block text-xs text-on-surface-muted">{selected.role}</span>
                </span>
              </>
            ) : (
              <span className="text-on-surface-muted flex-1">Choose a specialist…</span>
            )}
            <span className="material-symbols-outlined text-[18px] text-on-surface-muted shrink-0">
            ...
            <div className="absolute top-full mt-1 left-0 right-0 bg-surface-card border border-outline rounded-xl shadow-lg z-10 overflow-hidden">
              {DEMO_SPECIALISTS.map((s) => (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    <span className="block font-medium text-on-surface">{s.name}</span>
                    <span className="block text-xs text-on-surface-muted">{s.role}</span>
                  </span>
                  {selected?.id === s.id && (
