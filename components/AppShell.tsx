...
import { getCurrentSpecialist, clearCurrentSpecialist, StoredSpecialist } from "@/lib/queue/specialist"
import { getCurrentApplicant, clearCurrentApplicant, StoredApplicant } from "@/lib/queue/applicant"
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <IdentityContext.Provider value={{ specialist, applicant, requestLogin: handleRequestLogin }}>
      {showModal && (
        <LoginModal dismissible={!!(specialist || applicant)} onClose={handleModalClose} />
      )}
      {hideSidebar ? (
        <main className="flex-1 min-h-screen flex flex-col">
          {applicant && (
            <ApplicantNav applicant={applicant} onSwitchUser={handleRequestLogin} onLogout={handleLogout} />
          )}
          <div className="flex-1">{children}</div>
        </main>
      ) : (
        <>
          <Sidebar onRequestLogin={handleRequestLogin} onLogout={handleLogout} specialist={specialist} />
